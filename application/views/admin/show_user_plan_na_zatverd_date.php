...
<link rel="stylesheet" type="text/css" href="<? echo base_url() ?>application/views/admin/css/message.css">
	...
	$(function() {
		...
		$(".DatePicker").datepicker("setDate", "<?php echo $d_begin ?>");
...
if($planovi_zavd) {
	...
	<tfoot>
	  <tr>
	      <td colspan="6" style="text-align: center; font-size:18px;">Завдання річного плану</td>
...
<? if($status->flag == 1) {	?>
		...
		<input type="hidden" name="d_begin" value='<?php echo $d_begin ?>' />
		<input type="hidden" name="d_end" value='<?php echo $d_end ?>' />
...
<? if($status->flag != 1 and $status->flag != 2) {	?>
		...
		<input type="hidden" name="d_begin" value='<?php echo $d_begin ?>' />
		<input type="hidden" name="d_end" value='<?php echo $d_end ?>' />
		<input type="submit" class="button but orange" value="Затвердити факт виконання">
		</form>
<? } ?>	
<? } ?>
